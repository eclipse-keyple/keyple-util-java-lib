...
@AppScoped
class CardReaderViewModel @Inject constructor(private val schedulerProvider: SchedulerProvider) : ViewModel() {
    /* application states */
    enum class AppState {
    ...
    private var readersInitialized = true
        ...
        if (readersInitialized) {
            ...
            timer.schedule(object : TimerTask() {
                override fun run() {
                    ...
                    when(status){
                        Status.TICKETS_FOUND -> setUiResponse(
                            Status.TICKETS_FOUND,
                            7,
                            "Season Pass",
                            ""
                        )
                        Status.LOADING, Status.ERROR, Status.SUCCESS, Status.INVALID_CARD, Status.EMPTY_CARD -> setUiResponse(status, 0, "", "")
                    }
                }
    ...
    fun stopNfcDetection(activity: Activity?) {
        if (readersInitialized) {
            Timber.d("STUB: stopNfcDetection")
        }
    }
    ...
    companion object {
        ...
        private val EVENT_DELAY_MS = 3000
    }
}
